...
class PlusEventView(disnake.ui.View):
    ...
    async def build_embed(self, guild: disnake.Guild) -> disnake.Embed:
        ...
        # Весь блок (статус + время + список) лежит в description эмбеда —
        # там лимит 4096 симв., что с запасом вмещает 35 строк участников.
        desc = (
            f"{status} {e('TIME')}<t:{self.state.ts}:f> (<t:{self.state.ts}:R>)\n\n"
            f"**Участники:** {e('PEOPLE')}`{self.state.used}/{self.state.total_slots}`\n"
            f"{await self._participants_text(guild)}"
        )

        emb = brand_embed(
            title=f"{e('PLUS')}PLUS | {self.state.event_type.upper()}",
            description=desc,
        )
